...
if ($("#mainBnner").length) {
  ...
  // banner bg img set
  $(".style2 #mainBnner .owl-stage-outer .owl-stage .owl-item").each(
    function () {
...
$(".select-btn a").on("click", function () {
  var text_view = $(".facility-view .facility-box.active")
    .children(".inner-box")
    .children(".name")
    .text();
...
$(".booking-btnTop").on("click", function () {
  ...
  if ($(window).width() > 767) {
    body_scrollV =
      body_scrollV - $("#header").height() - $(".fiexd-nav").height();
  } else {
...
$(
  ".quck-linkRight .login-link,.quck-linkRight .registration,.quck-menuOverlay"
).on("click", function () {
  $("body").animate({ left: "0" }, 500);
  $(".quck-linkRight").animate({ right: "-390px" }, 500);
  $(".quck-menuOverlay").fadeOut(500);
...
if ($(".event-gallerSlider").length == 1) {
  $(".event-gallerSlider").owlCarousel({
    ...
    nav: true,
    navText: [
      '<img src="images/service-gallerLeft-arrow.png">',
      '<img src="images/service-gallerRight-arrow.png">',
    ],
    responsive: {
      0: {
        items: 1,
